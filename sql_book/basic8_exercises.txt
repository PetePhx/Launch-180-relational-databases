1. Make sure you are connected to the encyclopedia database. Add a column to the
...
ALTER TABLE animals ADD COLUMN class varchar(50);

UPDATE animals SET class = 'Aves';


2. Add two more columns to the animals table called phylum and kingdom. Both
should hold strings of up to 100 characters.
...
3. Add a column to the countries table called continent to hold strings of up to
50 characters.

Update all the rows in the table so France and Germany have a value of Europe
for this column, Japan has a value of Asia and the USA has a value of North
America.
...
4. In the celebrities table, update the Elvis row so that the value in the
deceased column is true. Then change the column so that it no longer allows NULL
values.
...
6. Change the name of the celebrities table to singers, and remove anyone who
isn't a singer.
...
8. Connect to the ls_burger database. Change the drink on James Bergman's order
...
9. Add Fries to Aaron Muller's order. Make sure to add the cost ($0.99) to the
...
10. The cost of Fries has increased to $1.20. Update the data in the table to
reflect this.
